...
object CdnUrls {
    /** Drops blank and duplicate candidates without normalizing signed URL bytes. */
    fun distinct(urls: List<String>): List<String> = urls.filter { it.isNotBlank() }.distinct()

    /** Prefers official non-peer hosts; keeps the API list when no preferred route is available. */
    fun officialCandidates(urls: List<String>): List<String> {
        val originals = distinct(urls)
        val preferred =
            originals.filter { url ->
        ...
        return preferred.ifEmpty { originals }
    }
